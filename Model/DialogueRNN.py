...
class MatchingAttention(nn.Module):
    ...
    def __init__(self, mem_dim, cand_dim, alpha_dim=None, att_type='general', device = 0):
    ...
    def forward(self, M, x, mask=None):
        ...
        if self.att_type=='dot':
            ...
            if type(mask)==type(None):
                mask = torch.ones(M.size(1), M.size(0)).type(M.type()).to(self.device)
            M_ = M.permute(1,2,0) # batch, mem_dim, seqlen
            x_ = self.transform(x).unsqueeze(1) # batch, 1, mem_dim
...
class DialogueRNNCell(nn.Module):
    ...
    def __init__(self, D_m, D_g, D_p, D_e, listener_state=False,
                            context_attention='simple', D_a=100, dropout=0.5, device = 0):
        ...
        if context_attention=='simple':
            ...
            self.attention = MatchingAttention(D_g, D_m, D_a, context_attention, device)
    ...
    def forward(self, U, qmask, g_hist, q0, e0):
        ...
        g_ = self.g_cell(torch.cat([U,q0_sel], dim=1),
                torch.zeros(U.size()[0],self.D_g).type(U.type()).to(self.device) if g_hist.size()[0]==0 else
                g_hist[-1])
        g_ = self.dropout(g_)
        if g_hist.size()[0]==0:
            c_ = torch.zeros(U.size()[0],self.D_g).type(U.type()).to(self.device)
            alpha = None
        else:
        ...
        e0 = torch.zeros(qmask.size()[0], self.D_e).type(U.type()).to(self.device) if e0.size()[0]==0\
                else e0
...
class DialogueRNN(nn.Module):
    ...
    def __init__(self, D_m, D_g, D_p, D_e, listener_state=False,
        ...
        self.dialogue_cell = DialogueRNNCell(D_m, D_g, D_p, D_e,
                            listener_state, context_attention, D_a, dropout, device)
        self.device = device
    ...
    def forward(self, U, qmask):
        ...
        g_hist = torch.zeros(0).type(U.type()).to(self.device) # 0-dimensional tensor
        q_ = torch.zeros(qmask.size()[1], qmask.size()[2],
                                    self.D_p).type(U.type()).to(self.device) # batch, party, D_p
        e_ = torch.zeros(0).type(U.type()).to(self.device) # batch, D_e
        e = e_
...
class BiModel(nn.Module):
    def __init__(self, D_m, D_g, D_p, D_e, D_h = 100,
                 dataset='MELD', n_classes=7, n_speakers=9, listener_state=False, 
                 context_attention='simple', D_a=100, dropout_rec=0.5, dropout=0.5, device=0):
        ...
        self.dialog_rnn_f = DialogueRNN(D_m, D_g, D_p, D_e,listener_state,
                                    context_attention, D_a, dropout_rec, device)
    ...
    def forward(self, U, qmask, umask):
        ...
        qmask -> seq_len, batch, party
        """
        # Bidirectional DialogueRNN for IEMOCAP, Unidirectional DialogueRNN for MELD
        if self.dataset == 'IEMOCAP':
            emotions_f, _ = self.dialog_rnn_f(U, qmask)
            rev_U = self._reverse_seq(U, umask)
            rev_qmask = self._reverse_seq(qmask, umask)
            emotions_b, _ = self.dialog_rnn_r(rev_U, rev_qmask)
            emotions_b = self._reverse_seq(emotions_b, umask)
            emotions = torch.cat([emotions_f, emotions_b],dim = -1)
            embeddings = self.relu(self.dropout_layer(self.linear(emotions)))
        else:
            emotions, _ = self.dialog_rnn_f(U, qmask)
            embeddings = self.relu(self.dropout_layer(emotions))
...
class Model(nn.Module):
    ...
    def __init__(self, D_m, D_g, D_p, D_e, D_h,
        ...
        self.matchatt = MatchingAttention(D_e,D_e,att_type='general2', device = 0)
