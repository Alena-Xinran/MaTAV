...
class TrainMultiEMO():
    ...
    def unweighted_accuracy(self,true_labels, predictions):
        ...
        for emotion in unique_labels:
            # 使用np.equal来比较两个数组中相应元素是否相等
            class_accuracy = accuracy_score(np.equal(true_labels, emotion), np.equal(predictions, emotion))
            class_accuracies[emotion] = class_accuracy
        
        # 计算未加权的平均准确率
        ua = np.mean(list(class_accuracies.values()))
        return ua, class_accuracies
    ...
    def train_or_eval_model_per_epoch(self, dataloader, train = True):
        ...
        for _, data in enumerate(dataloader):
            ...
            loss = soft_HGR_loss*self.HGR_loss_param + SWFC_loss * self.SWFC_loss_param 
        ...
        #avg_acc = round(self.unweighted_accuracy(all_labels, all_preds) * 100, 4)
        ua, category_accuracies = self.unweighted_accuracy(all_labels, all_preds)
        #report = classification_report(all_labels, all_preds, digits = 4)

        return round(total_loss, 4), round(ua*100,4), category_accuracies



    def train_or_eval_linear_model(self):
        for e in range(self.num_epochs):
            train_loss,  train_ua, train_category = self.train_or_eval_model_per_epoch(self.train_dataloader, train=True)
            with torch.no_grad():
                valid_loss, valid_ua, valid_category = self.train_or_eval_model_per_epoch(self.valid_dataloader, train=False)
                test_loss, test_ua, test_category = self.train_or_eval_model_per_epoch(self.test_dataloader, train=False)

            print(f'Epoch {e + 1}, Test loss: {train_loss},  WF1: {train_ua}')
            print(f"Test Category WF1: {train_category}")
            # print(f'Epoch {e + 1}, Valid loss: {valid_loss}, WF1: {valid_ua}')
            # print(f"Valid Category Accuracies: {valid_category}")
            # print(f'Epoch {e + 1}, Test loss: {test_loss}, UA: {test_ua}')
            # print(f"Test Category Accuracies: {test_category}")

            self.scheduler.step(train_loss)

            if train_ua >= self.best_train_acc:
                self.best_train_acc = train_ua
                self.best_train_category_acc = train_category  # 更新最佳模型的类别准确率
                self.best_epoch = e + 1

        print('Best test acc: {:.2f}% at epoch {}'.format(self.best_train_acc, self.best_epoch))
        print('Best test category accuracies:', self.best_train_category_acc)
