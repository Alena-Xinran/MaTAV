...
class TrainMultiEMO():
    ...
    def __init__(self, dataset, batch_size, num_epochs, learning_rate, weight_decay, 
                 ...
                 HGR_loss_param, multi_attn_flag, device):
    ...
    def train_or_eval_model_per_epoch(self, dataloader, train = True):
        ...
        total_loss = 0.0
        total_SWFC_loss, total_HGR_loss = 0.0, 0.0
        ...
        for _, data in enumerate(dataloader):
            ...
            labels = padded_labels[padded_labels != -1]
            print(labels.shape)
            fused_text_features, fused_audio_features, fused_visual_features, fc_outputs = \
            ...
            preds = torch.argmax(fc_outputs, dim = -1)
    ...
    #         padded_labels = padded_labels.reshape(-1)
    #         labels = padded_labels[padded_labels != -1]
    #         mask = (padded_texts != 0).any(dim=2)  # Assuming the last dimension holds features

    #         # Pass the mask to the model
    #         fused_text_features, fused_audio_features, fused_visual_features, fc_outputs, mlp_outputs = \
    #             self.model(padded_texts, padded_audios, padded_visuals, padded_speaker_masks, padded_utterance_masks, mask)
    ...
    def train_or_eval_linear_model(self):
        for e in range(self.num_epochs):
            train_loss, train_HGR_loss, train_SWFC_loss, train_f1, train_acc, _ = self.train_or_eval_model_per_epoch(self.train_dataloader, train = True)
            with torch.no_grad():
                valid_loss, valid_HGR_loss, valid_SWFC_loss,valid_f1, valid_acc, _ = self.train_or_eval_model_per_epoch(self.valid_dataloader, train = False)
                test_loss, test_HGR_loss, test_SWFC_loss, test_f1, test_acc, test_report = self.train_or_eval_model_per_epoch(self.test_dataloader, train = False)
            print('Epoch {}, train loss: {}, train HGR loss: {}, train SWFC loss: {},  train f1: {}, train acc: {}'.format(e + 1, train_loss, train_HGR_loss, train_SWFC_loss,  train_f1, train_acc))
            print('Epoch {}, valid loss: {}, valid HGR loss: {}, valid SWFC loss: {},  valid f1: {}, valid acc: {}'.format(e + 1, valid_loss, valid_HGR_loss, valid_SWFC_loss,  valid_f1, valid_acc))
            print('Epoch {}, test loss: {}, test HGR loss: {}, test SWFC loss: {},  test f1: {}, test acc: {}, '.format(e + 1, test_loss, test_HGR_loss, test_SWFC_loss, test_f1, test_acc))
            print(test_report)   
...
if __name__ == '__main__':
    ...
    multiemo_train = TrainMultiEMO(dataset, batch_size, num_epochs, learning_rate, 
                                   ...
                                   SWFC_loss_param, HGR_loss_param, multi_attn_flag, device)
