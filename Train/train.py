...
class TrainMultiEMO():
    ...
    def unweighted_accuracy(self,true_labels, predictions):
        ...
        # 计算每个类别的准确率并收集
        class_accuracies = [
            accuracy_score(true_labels == emotion, predictions == emotion)
        ...
        return ua
    def weighted_accuracy(self,true_labels, predictions):
        # 确保输入是numpy数组
        true_labels = np.array(true_labels)
        predictions = np.array(predictions)
        ...
        for emotion in unique_labels:
            # 创建一个类别的掩码，只考虑当前类别
            mask = true_labels == emotion
            # 计算当前类别的准确率
            class_accuracy = accuracy_score(true_labels[mask], predictions[mask])
            accuracies.append(class_accuracy)
            # 计算权重，即该类别在数据集中的比例
            weight = np.sum(mask) / len(true_labels)
            weights.append(weight)
        
        # 计算加权平均准确率
        weighted_acc = np.sum(np.array(accuracies) * np.array(weights))
        return weighted_acc
    ...
    def train_or_eval_model_per_epoch(self, dataloader, train = True):
        ...
        wa = round(self.weighted_accuracy(all_labels, all_preds) * 100, 4)
        ua = round(self.unweighted_accuracy(all_labels, all_preds) * 100, 4)
        report = classification_report(all_labels, all_preds, digits = 4)

        return round(total_loss, 4), round(total_HGR_loss, 4), round(total_SWFC_loss, 4), round(total_CE_loss, 4), wa, ua, report
    ...
    def train_or_eval_linear_model(self):
        for e in range(self.num_epochs):
            ...
            print('Epoch {}, train loss: {}, train HGR loss: {}, train SWFC loss: {}, train CE loss: {}, train wa: {}, train ua: {}'.format(e + 1, train_loss, train_HGR_loss, train_SWFC_loss, train_CE_loss, train_f1, train_acc))
            print('Epoch {}, valid loss: {}, valid HGR loss: {}, valid SWFC loss: {}, valid CE loss: {}, valid wa: {}, valid ua: {}'.format(e + 1, valid_loss, valid_HGR_loss, valid_SWFC_loss, valid_CE_loss, valid_f1, valid_acc))
            print('Epoch {}, test loss: {}, test HGR loss: {}, test SWFC loss: {}, test CE loss: {}, test wa: {}, test ua: {}, '.format(e + 1, test_loss, test_HGR_loss, test_SWFC_loss, test_CE_loss, test_f1, test_acc))
            print(test_report)   
